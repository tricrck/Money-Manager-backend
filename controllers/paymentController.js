 ...
 * @param {Object} req - Express request object
 * @param {Object} res - Express response object
 * @returns {Promise<Object>} Withdrawal transaction details or error response
 */
exports.processMpesaWithdrawal = async (req, res) => {
  try {
    const { phoneNumber, amount, reason, withdrawalPurpose, relatedItemId, relatedItemModel, metadata = {} } = req.body;
    ...
    // B2C transaction requires a valid reason
    // CommandID can be: BusinessPayment, SalaryPayment, or PromotionPayment
    ...
    // Generate timestamps and security credentials
    const timestamp = moment().format("YYYYMMDDHHmmss");
    const password = base64.encode(
    ...
    const transactionReference = `REF${Math.floor(Math.random() * 1000000000)}`;
    ...
    // Return both the B2C response and our transaction ID
    res.json({
      ...b2cResponse.data,
      transactionId: transaction._id
    });
 ...
 */
exports.mpesaWithdrawalCallback = async (req, res) => {
  try {
    ...
    if (!transaction) {
      console.log("Transaction not found for conversation ID:", originatorConversationId);
      return res.status(404).json({ error: "Transaction not found" });
    }

    // Extract details
    const resultDesc = resultData.ResultDesc;
    const workingAccountBalance = resultData.ResultParameters?.ResultParameter?.find(
      param => param.Key === "WorkingAccountBalance"
    )?.Value;
    const transactionId = resultData.ResultParameters?.ResultParameter?.find(
      param => param.Key === "TransactionID"
    )?.Value;
    const transactionAmount = resultData.ResultParameters?.ResultParameter?.find(
      param => param.Key === "TransactionAmount"
    )?.Value;
    const transactionReceipt = resultData.ResultParameters?.ResultParameter?.find(
      param => param.Key === "TransactionReceipt"
    )?.Value;
    const receiverPartyPublicName = resultData.ResultParameters?.ResultParameter?.find(
      param => param.Key === "ReceiverPartyPublicName"
    )?.Value;
    const completionTime = resultData.ResultParameters?.ResultParameter?.find(
      param => param.Key === "TransactionCompletedDateTime"
    )?.Value;
    ...
    if (resultCode === 0) {
      ...
      transaction.status = 'completed';
      transaction.transactionId = transactionId;
      transaction.completedAt = new Date(completionTime) || new Date();
      ...
      // Also save withdrawal to the main Payment collection with negative amount
      const payment = new Payment({
        userId: transaction.userId,
        paymentMethod: "M-Pesa",
        amount: -Math.abs(transactionAmount || transaction.amount), // Negative to indicate outflow
        currency: "KES",
        status: "success",
        paymentPurpose: transaction.withdrawalPurpose,
        relatedItemId: transaction.relatedItemId,
        relatedItemModel: transaction.relatedItemModel,
        transactionId: transactionId,
 ...
 * @returns {Promise<Object>} Acknowledgment response
 */
exports.mpesaWithdrawalTimeout = async (req, res) => {
  try {
    const { Result } = req.body;
    if (!Result) {
...
exports.checkWithdrawalStatus = async (req, res) => {
  try {
    ...
    if (transaction.status === 'processing' && transaction.createdAt < oneMinuteAgo) {
      try {
        const status = await queryMpesaTransactionStatus(transaction.conversationId, 'b2c');
        if (status) {
          transaction.status = status.success ? 'completed' : 'failed';
          transaction.resultCode = status.resultCode;
          transaction.resultDesc = status.resultDesc;
          ...
          if (status.success && status.transactionId) {
            ...
            const payment = new Payment({
              userId: transaction.userId,
              paymentMethod: "M-Pesa",
              amount: -Math.abs(transaction.amount), // Negative to indicate outflow
              currency: "KES",
              status: "success",
              paymentPurpose: transaction.withdrawalPurpose,
              relatedItemId: transaction.relatedItemId,
        ...
        console.error("Error querying M-Pesa withdrawal status:", error);
        // Don't update status on error, just continue
      }
    }
...
/**
 * Check M-Pesa account balance from the business till (Account Balance API)
 * @param {Object} req - Express request object
 * @param {Object} res - Express response object
 * @returns {Promise<Object>} Balance details or error response
 */
exports.checkMpesaBalance = async (req, res) => {
  try {
    const timestamp = require("moment")().format("YYYYMMDDHHmmss");
    // For account balance, the initiator password is used to generate the security credential
    const securityCredential = require("base-64").encode(process.env.MPESA_INITIATOR_PASSWORD);

    // Generate OAuth Token
    const authResponse = await require("axios").get(
      "https://sandbox.safaricom.co.ke/oauth/v1/generate?grant_type=client_credentials",
      {
        auth: {
          username: process.env.MPESA_CONSUMER_KEY,
    ...
    // Call the Account Balance API endpoint
    const balanceResponse = await require("axios").post(
      "https://sandbox.safaricom.co.ke/mpesa/accountbalance/v1/query",
      {
        Initiator: process.env.MPESA_INITIATOR_NAME,
        SecurityCredential: securityCredential,
        CommandID: "AccountBalance",
        PartyIdentifier: process.env.MPESA_SHORTCODE,
        IdentifierType: 4, // Typically, 4 indicates the shortcode or till number
    ...
    console.error("Error querying M-Pesa account balance:", error.response?.data || error.message);
    res.status(500).json({ error: error.response?.data || error.message });
  }
};
 ...
 * This endpoint is called when the request times out
 * @param {Object} req - Express request object
 * @param {Object} res - Express response object
 * @returns {Object} Success acknowledgment
 */
exports.handleBalanceTimeout = async (req, res) => {
  try {
    // Log the timeout response for debugging purposes
    console.log("M-Pesa Balance Timeout Response:", JSON.stringify(req.body));
    
    // Store the timeout event in database if needed
    // await db.timeoutEvents.create({ 
    //   type: 'account_balance',
    //   requestId: req.body.OriginatorConversationID,
    //   timestamp: new Date(),
    //   response: req.body
    // });
    
    // Always respond with a success message to M-Pesa
    // This acknowledges receipt of the timeout notification
    res.status(200).json({
      ResponseCode: "00000000",
      ResponseDesc: "Timeout notification received successfully"
    });
  } catch (error) {
    console.error("Error handling M-Pesa balance timeout:", error);
    // Still return a success response to M-Pesa even if we had an internal error
 ...
 * This endpoint receives the actual balance information
 * @param {Object} req - Express request object
 * @param {Object} res - Express response object
 * @returns {Object} Success acknowledgment
 */
exports.handleBalanceResult = async (req, res) => {
  try {
    // Log the result for debugging
    console.log("M-Pesa Balance Result Response:", JSON.stringify(req.body));
    
    // Extract balance information
    const resultParams = req.body.Result;
    
    // Process the balance results
    if (resultParams && resultParams.ResultCode === 0) {
      // Successful balance query
      const balanceInfo = resultParams.ResultParameters.ResultParameter;
      
      // Parse the balance information
      // The balance data usually comes as an array of name-value pairs
      const parsedBalance = {};
      if (Array.isArray(balanceInfo)) {
        balanceInfo.forEach(item => {
          parsedBalance[item.Key] = item.Value;
        });
      }
      
      // Store the balance information in database
      // await db.balanceResults.create({
      //   requestId: resultParams.OriginatorConversationID,
      //   resultType: resultParams.ResultType,
      //   resultCode: resultParams.ResultCode,
      //   resultDesc: resultParams.ResultDesc,
      //   transactionId: resultParams.TransactionID,
      //   accountBalance: parsedBalance,
      //   workingAccountBalance: parsedBalance.WorkingAccountAvailableFunds,
      //   timestamp: new Date()
      // });
      
      // You might want to emit an event or update UI in real-time
      // eventEmitter.emit('balance-update', parsedBalance);
    } else {
      // Failed balance query
      console.error("M-Pesa balance query failed:", resultParams?.ResultDesc || "Unknown error");
      
      // Log the failed attempt
      // await db.balanceErrors.create({
      //   requestId: resultParams?.OriginatorConversationID,
      //   resultCode: resultParams?.ResultCode,
      //   resultDesc: resultParams?.ResultDesc,
      //   timestamp: new Date()
    ...
    res.status(200).json({
      ResponseCode: "00000000",
      ResponseDesc: "Balance result received successfully"
    });
  } catch (error) {
    console.error("Error handling M-Pesa balance result:", error);
    // Still return a success response to M-Pesa
    res.status(200).json({
      ResponseCode: "00000000",
      ResponseDesc: "Received"
    });
